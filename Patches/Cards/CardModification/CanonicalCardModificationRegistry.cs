...
internal static class CanonicalCardModificationRegistry
{
    ...
    private static void RestoreFields(CardModel card, CanonicalCardBaseline baseline)
    {
        ...
        LoadoutCardKeywordState.ClearExplicitState(card);
    ...
    private static void Apply(CardModel canonical, CanonicalCardBaseline baseline, CardModificationDelta delta)
    {
        ...
        if (delta.KeywordOverrides.TryGetValue(LoadoutKeywords.XValueKey, out bool xValueOverride)
            ...
            costsX = true;
        if (delta.KeywordOverrides.TryGetValue(LoadoutKeywords.AltXValueKey, out bool altXValueOverride)
                ? altXValueOverride
                : baseline.Keywords.Contains(LoadoutKeywords.AltXValue))
            costsX = true;
        if (delta.KeywordOverrides.TryGetValue(LoadoutKeywords.AltAltXValueKey, out bool altAltXValueOverride)
                ? altAltXValueOverride
                : baseline.Keywords.Contains(LoadoutKeywords.AltAltXValue))
            costsX = true;
        ...
        {
            if (LoadoutCardKeywordState.HasEffectiveEntries(canonical, model.StorageKey))
                keywords.Add(model.Keyword);
            else
                keywords.Remove(model.Keyword);
        }
