...
public partial class NGenericSelectScreen : Control
{
    ...
    private readonly List<Control> _nonItemLayoutNodes = new();
    private readonly HashSet<Control> _nonItemLayoutNodeSet = new();
    ...
    private VBoxContainer? _bottomControlsContainer;
    private VBoxContainer? _bottomActionButtonsContainer;
    private VBoxContainer? _bottomCustomControlsContainer;
    private VBoxContainer? _bottomTogglesContainer;
    ...
    protected virtual int GetRemovalMaterializeBudget() => RemovalMaterializeBudget;
    protected virtual int GetScrollMaterializeBudget() => ScrollMaterializeBudget;
    protected virtual float GetMaterializeRowsBehind() => MaterializeRowsBehind;
    protected virtual float GetMaterializeRowsAhead() => MaterializeRowsAhead;
    protected virtual float GetRecycleRowsBehind() => RecycleRowsBehind;
    protected virtual float GetRecycleRowsAhead() => RecycleRowsAhead;
    ...
    protected virtual IReadOnlyList<IGenericSelectItem> BuildHiddenPrewarmItemList()
    {
        ...
        return _itemLayoutOrder
            ...
            .Take(GetInitialMaterializeBudget())
            .ToList();
    }
    ...
    public void AddCustomSidebarBottomControl(Control control)
    {
        if (_bottomCustomControlsContainer is null)
        {
            _pendingCustomSidebarBottomControls.Add(control);
            return;
        }

        _bottomCustomControlsContainer.AddChild(control);
    }
    ...
    public void ClearCustomSidebarControls()
    {
        ...
        foreach (Control pendingControl in _pendingCustomSidebarBottomControls)
        {
            if (GodotObject.IsInstanceValid(pendingControl) && pendingControl.GetParent() is null)
                pendingControl.QueueFree();
        }

        _pendingCustomSidebarBottomControls.Clear();
        ...
        {
            foreach (Node child in _bottomCustomControlsContainer.GetChildren())
            {
                _bottomCustomControlsContainer.RemoveChild(child);
                child.QueueFree();
            }
        }
    }
    ...
    private void ResumeRetainedLayout()
    {
        ...
        {
            MaterializeViewportItemViews(GetInitialMaterializeBudget(), updateExistingViews: false);
        }
    ...
    private void RebuildCurrentLayoutAfterSingleRemoval(bool updateExistingViews)
    {
        ...
        if (_materializationMode == SelectMaterializationMode.Lazy)
            MaterializeViewportItemViews(GetRemovalMaterializeBudget(), updateExistingViews);
        else
    ...
    private void BuildUtilityContainersIfMissing()
    {
        ...
        Control? bottomSpacer = _filterControls.GetNodeOrNull<Control>("SidebarBottomSpacer");
        if (bottomSpacer is null)
        {
            bottomSpacer = new Control
            {
                Name = "SidebarBottomSpacer",
                MouseFilter = MouseFilterEnum.Ignore,
                SizeFlagsHorizontal = SizeFlags.ExpandFill,
                SizeFlagsVertical = SizeFlags.ExpandFill
            };
            _filterControls.AddChild(bottomSpacer);
        }

        _bottomControlsContainer = _filterControls.GetNodeOrNull<VBoxContainer>("BottomControls");
        if (_bottomControlsContainer is null)
        {
            _bottomControlsContainer = new VBoxContainer
            {
                Name = "BottomControls",
                SizeFlagsHorizontal = SizeFlags.ExpandFill
            };
            _filterControls.AddChild(_bottomControlsContainer);
        }

        _bottomActionButtonsContainer = _bottomControlsContainer.GetNodeOrNull<VBoxContainer>("BottomActionButtons");
        if (_bottomActionButtonsContainer is null)
        {
            _bottomActionButtonsContainer = new VBoxContainer
            {
                Name = "BottomActionButtons",
                SizeFlagsHorizontal = SizeFlags.ExpandFill
            };
            _bottomControlsContainer.AddChild(_bottomActionButtonsContainer);
        }

        _bottomCustomControlsContainer = _bottomControlsContainer.GetNodeOrNull<VBoxContainer>("BottomCustomControls");
        if (_bottomCustomControlsContainer is null)
        {
            _bottomCustomControlsContainer = new VBoxContainer
            {
                Name = "BottomCustomControls",
                SizeFlagsHorizontal = SizeFlags.ExpandFill
            };
            _bottomControlsContainer.AddChild(_bottomCustomControlsContainer);
        }

        _bottomTogglesContainer = _bottomControlsContainer.GetNodeOrNull<VBoxContainer>("BottomToggles");
        if (_bottomTogglesContainer is null)
        {
            _bottomTogglesContainer = new VBoxContainer
            {
                Name = "BottomToggles",
                SizeFlagsHorizontal = SizeFlags.ExpandFill
            };
            _bottomControlsContainer.AddChild(_bottomTogglesContainer);
        }
        ...
        _bottomControlsContainer.SizeFlagsVertical = SizeFlags.ShrinkEnd;
        _bottomControlsContainer.Alignment = BoxContainer.AlignmentMode.End;
        _bottomActionButtonsContainer.SizeFlagsVertical = SizeFlags.ShrinkEnd;
        _bottomCustomControlsContainer.SizeFlagsVertical = SizeFlags.ShrinkEnd;
        _bottomTogglesContainer.SizeFlagsVertical = SizeFlags.ShrinkEnd;
        ...
        if (bottomSpacer.GetParent() == _filterControls)
            _filterControls.MoveChild(bottomSpacer, _filterControls.GetChildCount() - 1);

        if (_bottomControlsContainer.GetParent() == _filterControls)
            _filterControls.MoveChild(_bottomControlsContainer, _filterControls.GetChildCount() - 1);

        if (_bottomActionButtonsContainer.GetParent() == _bottomControlsContainer && _bottomCustomControlsContainer.GetParent() == _bottomControlsContainer)
            _bottomControlsContainer.MoveChild(_bottomCustomControlsContainer, _bottomActionButtonsContainer.GetIndex() + 1);

        if (_bottomCustomControlsContainer.GetParent() == _bottomControlsContainer && _bottomTogglesContainer.GetParent() == _bottomControlsContainer)
            _bottomControlsContainer.MoveChild(_bottomTogglesContainer, _bottomCustomControlsContainer.GetIndex() + 1);
    }
    ...
    private void ClearLayoutTracking()
    {
        ...
        _nonItemLayoutNodes.Clear();
        _nonItemLayoutNodeSet.Clear();
        _itemLayouts.Clear();
    ...
    private void RebuildCustomControls()
    {
        ...
        if (_bottomCustomControlsContainer is not null)
        {
            _bottomCustomControlsContainer.SizeFlagsHorizontal = SizeFlags.ExpandFill;

            foreach (Control control in _pendingCustomSidebarBottomControls.ToList())
            {
                if (!GodotObject.IsInstanceValid(control))
                    continue;
    ...
    private static Container? GetSidebarSectionContainer(
        SelectSidebarSection section,
        Container? mainContainer,
        Container? bottomContainer)
    {
        return section == SelectSidebarSection.Bottom
    ...
    {
        if (_actionButtonsContainer is null && _bottomActionButtonsContainer is null)
            return;
        ...
        {
            foreach (Node child in _bottomActionButtonsContainer.GetChildren())
                child.QueueFree();
        }
        ...
        bool hasMainActionButtons = _actionButtons.Any(actionButton => actionButton.Section == SelectSidebarSection.Main);
        if (hasMainActionButtons && _actionButtonsContainer is not null)
        ...
        {
            Container? container = GetSidebarSectionContainer(
                actionButton.Section,
                _actionButtonsContainer,
                _bottomActionButtonsContainer);
    ...
    {
        if (_togglesContainer is null && _bottomTogglesContainer is null)
            return;

        if (_togglesContainer is not null)
        {
            foreach (Node child in _togglesContainer.GetChildren())
                child.QueueFree();
        }

        if (_bottomTogglesContainer is not null)
        {
            foreach (Node child in _bottomTogglesContainer.GetChildren())
                child.QueueFree();
        }
    ...
    private void TrackVisibleLayoutNode(Control control)
    {
        ...
        if (!_activationItemsByView.ContainsKey(control) && _nonItemLayoutNodeSet.Add(control))
            _nonItemLayoutNodes.Add(control);
    }
    ...
    private void UntrackVisibleLayoutNode(Control control)
    {
        ...
        if (_nonItemLayoutNodeSet.Remove(control))
            _nonItemLayoutNodes.Remove(control);
        _lastCullNodeCount = -1;
    }
    ...
    private int MaterializeViewportItemViews(int maxCount, bool updateExistingViews = false)
    {
        ...
        materialized += MaterializeLayoutWindow(BuildViewportMaterializeRect(_targetScrollY, GetMaterializeRowsBehind(), GetMaterializeRowsAhead()), maxCount - materialized, updateExistingViews);
        return materialized;
    }
    ...
    /// <summary>
    /// Allows a specialized screen to incrementally warm a bounded layout window
    /// without exposing the generic screen's layout dictionaries.
    /// </summary>
    protected int MaterializeSpecializationWindow(
        float scrollY,
        float rowsBehind,
        float rowsAhead,
        int maxCount,
        bool updateExistingViews = false)
    {
        ...
        Rect2 materializeRect = BuildViewportMaterializeRect(scrollY, rowsBehind, rowsAhead);
        float top = materializeRect.Position.Y;
        float bottom = materializeRect.End.Y;
        int materialized = 0;
        ...
        for (int i = FindFirstLayoutIndexAtOrAfter(top); i < _itemLayoutOrder.Count; i++)
        {
            ...
            if (!MaterializeItemView(item, layout, updateExistingViews))
                continue;

            if (needsView)
            {
                if (item.View is Control view && GodotObject.IsInstanceValid(view))
                    SetLayoutNodeActive(view, IsLayoutNearViewport(layout));
    ...
    /// <summary>
    /// Incrementally walks the ordered layout and materializes at most maxCount
    /// missing views. The cursor survives between frames and wraps at the end.
    /// </summary>
    protected int MaterializeSpecializationFromCursor(
        ref int cursor,
        int maxCount,
        bool updateExistingViews = false)
    {
        ...
        cursor = Math.Clamp(cursor, 0, Math.Max(0, _itemLayoutOrder.Count - 1));
        int materialized = 0;
        int scanned = 0;
        int itemCount = _itemLayoutOrder.Count;

        while (scanned < itemCount && materialized < maxCount)
        {
            IGenericSelectItem item = _itemLayoutOrder[cursor];
    ...
    private bool IsLayoutNearViewport(SelectItemLayout layout)
    {
        ...
        float behindBuffer = rowHeight * GetMaterializeRowsBehind();
        float aheadBuffer = rowHeight * GetMaterializeRowsAhead();
    ...
    private void UpdateViewportCulling(bool force = false)
    {
        ...
        ApplyViewportCulling(cullTop, cullBottom);
        RecycleDistantItemViews();
    }

    /// <summary>
    /// Default culling walks every materialized layout node. Heavy specialized
    /// screens may override this and use the ordered layout helpers below so the
    /// work remains proportional to the viewport rather than the retained cache.
    /// </summary>
    protected virtual void ApplyViewportCulling(float cullTop, float cullBottom)
    {
        for (int i = _visibleLayoutNodes.Count - 1; i >= 0; i--)
        {
            ...
            if (!GodotObject.IsInstanceValid(control))
            {
                ...
                _nonItemLayoutNodeSet.Remove(control);
                _nonItemLayoutNodes.Remove(control);
                continue;
            }

            SetLayoutNodeActive(control, IsControlInsideVerticalWindow(control, cullTop, cullBottom));
        }
    }

    /// <summary>
    /// Culls only generated headers and other non-item layout controls. Specialized
    /// item screens can combine this with CollectMaterializedItemViewsInWindow.
    /// </summary>
    protected void CullNonItemLayoutNodes(float cullTop, float cullBottom)
    {
        for (int i = _nonItemLayoutNodes.Count - 1; i >= 0; i--)
        {
            Control control = _nonItemLayoutNodes[i];
            if (!GodotObject.IsInstanceValid(control))
            {
                _nonItemLayoutNodes.RemoveAt(i);
                _nonItemLayoutNodeSet.Remove(control);
                _visibleLayoutNodeSet.Remove(control);
                _visibleLayoutNodes.Remove(control);
                continue;
    ...
    /// <summary>
    /// Collects only already-created item views whose queued layout intersects the
    /// requested vertical window. The binary-search start keeps retained-screen
    /// culling proportional to visible rows even after hundreds of views exist.
    /// </summary>
    protected void CollectMaterializedItemViewsInWindow(
        float cullTop,
        float cullBottom,
        ISet<Control> destination)
    {
        if (_itemLayoutOrder.Count == 0)
            return;

        for (int i = FindFirstLayoutIndexAtOrAfter(cullTop); i < _itemLayoutOrder.Count; i++)
        {
            IGenericSelectItem item = _itemLayoutOrder[i];
            if (!_itemLayouts.TryGetValue(item, out SelectItemLayout layout))
                continue;

            if (layout.Position.Y > cullBottom)
                break;

            if (layout.Position.Y + layout.Size.Y < cullTop)
                continue;
    ...
    private static bool IsControlInsideVerticalWindow(Control control, float cullTop, float cullBottom)
    {
        ...
        return bottom >= cullTop && top <= cullBottom;
    }
    ...
    private void UpdateSelectScroll(double delta)
    {
        ...
        if (_materializationMode == SelectMaterializationMode.Lazy)
        {
            MaterializeViewportItemViews(GetScrollMaterializeBudget());
...
public sealed class SelectScreenBuilder<TModel>
{
    ...
    public SelectScreenBuilder<TModel> Toggle(
        string id,
        string label,
    ...
    public SelectScreenBuilder<TModel> ActionButton(
        ...
        SelectSidebarSection section = SelectSidebarSection.Main)
    {
        _screen.AddActionButton(new SelectActionButtonDefinition(id, label, onPressed, icon, section));
        return this;
    }
...
public enum SelectSidebarSection
{
    Main,
    Bottom
}
